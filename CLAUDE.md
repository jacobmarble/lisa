...
```
User creates FRD → /lisa skill converts to lisa.json → lisa.sh runs iterations → Agent implements stories
```
...
3. Agent reads `lisa.json`, finds first task where `passes: false`
4. Agent implements the task, runs verification, commits if passing
5. Agent updates `lisa.json` to mark task complete
...
- **lisa.json**, **progress.txt**, **.lisa/** are in the target project's working directory
...
- **Progress persistence**: `progress.txt` and `lisa.json` are the only state between iterations.
...
1. Create a simple test project with a small `lisa.json`
